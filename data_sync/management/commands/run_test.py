...
class Command(BaseCommand):
    def run_data_transformation(self):
        ...
        if token_verification_data['data']['status_code'] == 200:
            print('verified')
            secret_key_verification_data = convert_string_to_json(
                secret_key_verification())
            if secret_key_verification_data['data']['status_code'] == 200:
                print('verified')
                schema_verification_data = schema_verification()
                print('schema_verification_data output',
                      schema_verification_data, type(schema_verification_data))
                ...
                print('schema_verification_data status code',
                      schema_verification_data['data'], )
                print("type(schema_verification_data['data'])", type(
                    schema_verification_data['data']))
